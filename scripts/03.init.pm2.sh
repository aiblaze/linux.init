...
if command_exists "pm2"; then
    ...
    if confirm_action "Do you want to update PM2 to the latest version?"; then
        log "Updating PM2..."
        execute "npm install -g pm2@latest" "Failed to update PM2" "PM2 updated successfully"
    else
    ...
    execute "npm install -g pm2" "Failed to install PM2" "PM2 installed successfully"
fi
